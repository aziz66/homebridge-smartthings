...
### Fixed
- **Sticky `online=false` on devices whose initial `/health` probe returns OFFLINE** (#33): `MultiServiceAccessory.checkDeviceHealth()` ran exactly once in the constructor and set `this.online` based on that single response. Once false, every read short-circuited via `baseService.ts:getStatus()` and HomeKit got bogus values (e.g. literal `0` from `thermostatService.getCurrentTemperature()` when status fetch was blocked) — even though `/status` polls kept succeeding. The polling-loop's recovery branch (`multiServiceAccessory.ts:691-702`) only triggered after `failureCount ≥ 5`, which the failed health check never reached. The success path of `refreshStatus()` now resets `this.online = true`, `this.failureCount = 0`, and `this.giveUpTime = 0` on every successful `/status` call. The `/health` probe stays as advisory startup logging; the failureCount mechanism remains the safety net for genuine outages. **Bites locally-executing SmartThings Edge drivers hardest** (cloud `/health` does not reliably reflect local-execution device state) but applies to any startup-time health check that returns OFFLINE.
- **Heating-only thermostats lost `thermostatMode` at service registration** (#33): The `[temperatureMeasurement, thermostatHeatingSetpoint]` combo at `multiServiceAccessory.ts` had no optional capabilities, so devices like the Stelpro ST218 (and many other heating-only Zigbee/Z-Wave thermostats that expose mode + heat setpoint without a cooling setpoint) matched the heater combo and had `thermostatMode` stripped from the service's capability array. `thermostatService.ts` then unconditionally returned OFF/AUTO at three call sites (`getTargetHeatingCoolingState`, `setTargetHeatingCoolingState`, `getCurrentHeatingCoolingState`) and silently no-op'd mode-change commands. Heater combo now declares `thermostatMode`, `thermostatFanMode`, and `thermostatOperatingState` as `optionalCapabilities` so they are picked up when the device exposes them. Combo sort order (longest-required-first) preserves existing behavior for full Samsung thermostats (mode + heat + cool + temp combo still wins).
- **`Target Temperature` "exceeded minimum of 10" warning on frost-protection setpoints** (#33): `thermostatService.ts` never called `setProps()` on the `TargetTemperature` characteristic, so HomeKit used its default `minValue: 10`/`maxValue: 38`/`minStep: 0.1`. Stelpro and many EU/Canada baseboard/in-floor thermostats use 5 °C frost protection, which tripped the warning every poll. Now sets `{ minValue: 5, maxValue: 36, minStep: 0.5 }` to cover frost protection through residential HVAC max, with HomeKit's standard 0.5 °C slider step (devices reporting finer resolution still work — HomeKit snaps on the user side, reads can return any value). Bounds are in °C per HAP spec; the existing °F conversion paths handle Fahrenheit-display devices unchanged.
